...
### Changed
- **TUI `ProjectList` refactor**: removed unreachable dead JSX block (`{!filterMode && filter}`), extracted shared `handleNavigate()` to eliminate duplicated up/down/enter logic, moved `setCursor`/`setScrollOffset` side effects out of `setFilter` updater callbacks (React purity), and memoized `totalWarnings` with `useMemo`.
- **TUI `Audit` memoization**: wrapped `byseverity`/`items` derivation in `useMemo([scanResult.projects])` so the warning list is only recomputed when the scan result changes, not on every keystroke.
...
### Tests
- **`merger.ts:340` parse-failure skip (`!file.data` branch)**: New test covers a `SettingsFile` with `exists=true` but no `data` (parse failure scenario). The `!file.exists` case was already tested; this independently covers the `!file.data` side of the compound short-circuit guard.
...
### Tests
- **`init.ts:111-118` unknown preset**: New test passes `preset: "bogus"` to `initCommand`, verifying `process.exit(1)` is called — the invalid-scope and invalid-mode guards were already tested but this `PRESETS[preset]` check was missing.
